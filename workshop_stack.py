                    ...
                    "phases": {
                        ...
                        "build": {
                            "commands": [
                                ...
                                "comm -13 <(sort from_s3_$KEY) <(sort $KEY) > arns",
                                "cat arns",
                                "arns_list=$(cat arns)",
                                "echo $arns_list",
                                "for arn in $arns_list; do RULE_NAME=$(echo $arn | cut -d'/' -f2 ); FILTER=$(jq -n -c --arg arn \"$arn\" '{resource: {eq: [$arn]}}'); aws accessanalyzer create-archive-rule --analyzer-name $ANALYZER_NAME --rule-name $RULE_NAME --filter $FILTER; done",
                                "for arn in $arns_list; do RULE_NAME=$(echo $arn | cut -d'/' -f2 ); aws accessanalyzer apply-archive-rule --analyzer-arn $ANALYZER_ARN --rule-name $RULE_NAME; done",
                                "aws accessanalyzer list-archive-rules --analyzer-name $ANALYZER_NAME",
                                "cat ${KEY} >> from_s3_${KEY}",
                                "cat from_s3_${KEY} | sort | uniq | sort > ${KEY}",
                                "cat ${KEY}",
                                "aws s3api put-object --bucket ${BUCKET} --key ${KEY} --body ${KEY}",
                            ]
                        },
                    },
                }
